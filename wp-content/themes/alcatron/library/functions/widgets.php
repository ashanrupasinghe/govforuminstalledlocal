...
class alc_flickr_widget extends WP_Widget {
	...
	function alc_flickr_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Display images from flickr', 'Alcatron')
		);
		...
		// Create the widget.
		$this->WP_Widget('alc-flickr-widget', __('Alcatron - Flickr images', 'Alcatron') , $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'Alcatron'); ?></label>
			<input id="<?php echo $this->get_field_id('title'); ?>" type="text" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" class="widefat" />
		</p>
        
		<p>
			<label for="<?php echo $this->get_field_id('flickr_id'); ?>"><?php _e('Flickr ID:', 'Alcatron'); ?></label> 
        ...
        <p>
			<label for="<?php echo $this->get_field_id('flickr_nr'); ?>"><?php _e('Number of photos:', 'Alcatron'); ?></label> 
...
class Latest_Tweets extends WP_Widget {
	...
	function Latest_Tweets() {
		...
		$this->WP_Widget( 'latest-tweets-widget','Alcatron - Latest Tweets', $widget_ops, $control_ops );
	}
...
<li class="twitter-item"><i class="icon-twitter"></i>';
		                    ...
		                    if($update) {
		                      ...
		                      if ( ( abs( time() - $time) ) < 86400 )
		                        $h_time = sprintf( __('%s ago', 'Alcatron'), human_time_diff( $time ) );
		                      else
		                        $h_time = date(__('Y/m/d', 'Alcatron'), $time);
		                      echo sprintf( __('%s', 'twitter-for-wordpress', 'Alcatron'),' <span class="twitter-timestamp"><abbr title="' . date(__('Y/m/d H:i:s', 'Alcatron'), $time) . '">' . $h_time . '</abbr></span>' );
		                     }
		                    echo '</li>
';
		                    $i++;
...
';
				...
				{ 
					echo _e('Sorry , Twitter seems down or responds slowly.', 'Alcatron'); 
				}
            ?>
		<?php
		...
		echo $before_title; ?>
			<a href="http://twitter.com/<?php echo $twitter_username  ?>"><?php echo $title ; ?></a>
		<?php echo $after_title._e('You need to Setup Twitter API OAuth settings first', 'Alcatron');
		echo $after_widget;
	}
}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__('@Follow Me' , 'Alcatron') , 'no_of_tweets' => '5', 'twitter_username'=>'weblusive', 'consumer_key' => '', 'consumer_secret' => '', 'access_token' => '', 'access_token_secret' => '');
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_username' ); ?>"><?php _e('Username', 'Alcatron')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'consumer_key' ); ?>"><?php _e('Consumer Key', 'Alcatron')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'consumer_secret' ); ?>"><?php _e('Consumer Secret', 'Alcatron')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'access_token' ); ?>"><?php _e('Access Token', 'Alcatron')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'access_token_secret' ); ?>"><?php _e('Access Token Secret', 'Alcatron')?> : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_tweets' ); ?>"><?php _e('Number of Tweets to show', 'Alcatron')?> : </label>
...
/* ***************************************************** 
 * Plugin Name: Alcatron Last Works
...
class alc_works_widget extends WP_Widget {
	...
	function alc_works_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Display latest works (Portoflio)', 'Alcatron')
		);

		// Create the widget.
		$this->WP_Widget('alc-works-widget', __('Alcatron - Latest Works', 'Alcatron') , $widget_ops);
	}
	...
	function form($instance) {
		...
		$instance = wp_parse_args((array)$instance, $defaults); ?>
		<p>
			<label for="<?php echo $this->get_field_id('post_title'); ?>"><?php _e('Title', 'Alcatron'); ?></label>
         ...
         <p>
			<label for="<?php echo $this->get_field_id('post_category'); ?>"><?php _e('Category', 'Alcatron'); ?></label> 
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_count'); ?>"><?php _e('Number of Posts to show', 'Alcatron'); ?></label> 
...
/* ***************************************************** 
 * Plugin Name: Alcatron Blog Posts
...
class alc_blogposts_widget extends WP_Widget {
	...
	function alc_blogposts_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Display latest blog posts on footer', 'Alcatron')
		);

		// Create the widget.
		$this->WP_Widget('alc-blogposts-widget', __('Alcatron Footer Blog Posts', 'Alcatron') , $widget_ops);
	}
	...
	function form($instance) {
        ...
        <p>
         ...
         <p>
			<label for="<?php echo $this->get_field_id('post_category'); ?>"><?php _e('Category (Leave Blank to show from all categories)', 'Alcatron'); ?></label> 
			<input id="<?php echo $this->get_field_id('post_category'); ?>" type="text" name="<?php echo $this->get_field_name('post_category'); ?>" value="<?php echo $instance['post_category']; ?>" class="widefat" />
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id('post_thumbs'); ?>"><?php _e('Show thumbnails', 'Alcatron'); ?></label>
...
class alc_totalposts_widget extends WP_Widget {
	...
	function alc_totalposts_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Retrieve and display popular/latest posts/latest comments.', 'Alcatron')
		);

		// Create the widget.
		$this->WP_Widget('alc-totalposts-widget', __('Alcatron Popular/Latest posts/Last comments', 'Alcatron') , $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		<div class="section-container tabs" data-section="tabs">
                   <section class="section">
                            <p class="title"><a href="#"><?php _e('Popular', 'Alcatron')?></a></p>
                            <div class="content">
                                <ul class="widget-popular-list">
                    ...
                    <section class="section">
                            <p class="title"><a href="#"><?php _e('Recent', 'Alcatron')?></a></p>
                            <div class="content">
                    ...
                    <section class="section">
                            <p class="title"><a href="#"><?php _e('Comments', 'Alcatron')?></a></p>
                            <div class="content">
                                <ul class="widget-post-list widget-comment-list">
					<?php 
					global $wpdb;	
					...
					foreach ($comments as $comment) :?>
						<li>
							<div class="wpl-image avatar-listing">
								<a href="<?php echo get_permalink($comment->ID).'#comment-'.$comment->comment_ID?>" title="<?php echo strip_tags($comment->comment_author).' '.__('on ', 'Alcatron').' '.$comment->post_title?>">
...
/* ***************************************************** 
 * Plugin Name: Alcatron Contact Widget
...
class alc_Contact_Form extends WP_Widget {
	...
	function alc_Contact_Form() {
		$widget_ops = array('classname' => 'alc_contact_form_entries', 'description' => __("Contact widget", 'Alcatron') );
		$this->WP_Widget('alc_Contact_Form', __('Alcatron - Contact Form', 'Alcatron'), $widget_ops);
	}

	function widget($args, $instance) {
		extract($args);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Contact Us', 'Alcatron') : $instance['title'], $instance);
		$email = apply_filters('widget_title', empty($instance['email']) ? __('', 'Alcatron') : $instance['email'], $instance);
		$success = apply_filters('widget_title', empty($instance['success']) ? __('Thank you, e-mail sent.', 'Alcatron') : $instance['success'], $instance);
		...
		if ( $title ) echo $before_title . $title . $after_title;
			...
			echo '<div class="small-4 columns right"><input type="submit" id="wformsend" value="'.__('Send', 'Alcatron').'" class="button" name="wsend"  /></div>';
	...
	?>
		...
		<div>
        	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:<br />', 'Alcatron'); ?>
			<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
		</div>
        <div>
        	<label for="<?php echo $this->get_field_id('email'); ?>"><?php _e('Email Address:<br />', 'Alcatron'); ?>
        ...
        <div>
        	<label for="<?php echo $this->get_field_id('success'); ?>"><?php _e('Success Message:<br />', 'Alcatron'); ?>
