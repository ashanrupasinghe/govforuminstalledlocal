...
<div class="row">
    <div class="large-12 columns main-content-top">
        <div class="row">
            ...
            <div class="large-6 columns">
                <?php $title_ws_2 = get_field ( 'workshop-2', 'option' );?>
                    <?php if(class_exists('the_breadcrumb')){ //$albc = new the_breadcrumb;

                    $post_type= get_post_type();
                    $achiveurl= get_post_type_archive_link($post_type);
                    echo '<ul class="breadcrumbs right"><li class="home"><a href="'.home_url().'">'.__('Home', 'Alcatron').'</a></li><li><a href="'.$achiveurl.'">'.$title_ws_2.'</a></li><li>&nbsp;&nbsp;'.get_the_title().'</li></ul>';
					} ?>
...
<div class="content_wrapper">
    ...
    <?php $title=get_field('workshop-2','option');
    	  $limit=wp_count_posts('workshop-2');
    	  $published_posts = wp_count_posts('workshop-2')->publish; 
    	  $icon_ws_2 = get_field ( 'icon-workshop-2', 'option' );
    	echo do_shortcode('[row][one_whole][workshop2 title="'.$title.'" limit="'.$published_posts.'" featured="0" icon="'.$icon_ws_2.'"/][/one_whole][/row]');?>
    </div></div>
</div>
<?php get_footer(); ?>