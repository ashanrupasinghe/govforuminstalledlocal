...
<div class="row">
    <div class="large-12 columns main-content-top">
        <div class="row">
            ...
            <div class="large-6 columns">
                <?php $title_ws_6 = get_field ( 'workshop-6', 'option' );?>
                    <?php if(class_exists('the_breadcrumb')){ //$albc = new the_breadcrumb;
                    $post_type= get_post_type();
                    $achiveurl= get_post_type_archive_link($post_type);
                    echo '<ul class="breadcrumbs right"><li class="home"><a href="'.home_url().'">'.__('Home', 'Alcatron').'</a></li><li><a href="'.$achiveurl.'">'.$title_ws_6.'</li><li>&nbsp;&&nbsp;'.get_the_title().'</li></ul>';
					} ?>
...
<div class="content_wrapper">
    ...
    <?php $title=get_field('workshop-6','option');
    	  $limit=wp_count_posts('workshop-6');
    	  $published_posts = wp_count_posts('workshop-6')->publish; 
    	  $icon_ws_6 = get_field ( 'icon-workshop-6', 'option' );
    	echo do_shortcode('[row][one_whole][workshop6 title="'.$title.'" limit="'.$published_posts.'" featured="0" icon="'.$icon_ws_6.'"/][/one_whole][/row]');?>
    </div></div>
</div>
<?php get_footer(); ?>