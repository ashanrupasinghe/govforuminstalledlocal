<?php 
$pageId = $post->ID;
$_SESSION['Alcatron_page_id'] = $pageId;
		...
		<div class="large-12 columns">
			<ul class="splitter portfolio-main filter">
				<?php if ($paginationEnabled):?>
					<li class="active"><a href="<?php echo get_page_link($pageId) ?>" class="all"><?php _e('All', 'Alcatron')?></a></li>
					<?php 
					...
					?>
				<?php else: ?>
					<li class="active"><a href="javascript:void(0)"  class="all"><?php _e('All', 'Alcatron')?></a></li>
					<?php 
						$cats = get_post_meta($post->ID, "_page_portfolio_cat", $single = true);                                                 
		...
		<div class="large-12 columns">
			...
			<?php if( $cats == '' ): ?>
				<p><?php _e('No categories selected. To fix this, please login to your WP Admin area and set
					...
					in the multi checkbox field "Portfolio Categories".', 'Alcatron')?>
				</p>
			<?php else: ?>		
				<?php 
		...
		<?php if ($paginationEnabled ):?>
			<?php if ( $wp_query->max_num_pages > 1 ): ?>
				<div class="large-10 columns">
					<?php include(Alcatron_PLUGINS . '/wp-pagenavi.php' ); wp_pagenavi(); ?> 
					<div class="clear"></div>
				</div>
			<?php endif?>
		<?php endif?>		
